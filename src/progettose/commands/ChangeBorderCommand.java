...
/**
 * Concrete command for changing the border color of all selected shapes on the
 * canvas.
 *
 */
public class ChangeBorderCommand extends Command {
     ...
     */
    public ChangeBorderCommand(CanvasClass app, Receiver receiver) {
        super(app, receiver);
    }
    ...
    public boolean execute() {
        ...
        for (Group a : selectedshapes) {
            if (!(a.getChildren().get(0) instanceof Text)) {
                this.receiver.modifyShape(shapesoncanvas.get(a.getChildren().get(0)), "CHANGE_BORDER");
            }
